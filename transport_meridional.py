			...
			if inn.vke.mask[ik,j,i] == False:
				if rbek[j,i] == 4. or rbek[j,i] == 5. or rbek[j,i] == 3. or rbek[j,i] == 2. or rbek[j,i] == 1.:
					...
					if lbrei >= 0 and lbrei <= 65:
						...
						full_tr[k,lbrei] = full_tr[k,lbrei] + inn.vke[ik,j,i]*dlxv[j,i]*ddue[ik,j,i] # Output in Sverdrup [m^3 s^-1]
						if i >= (cline_x[j] -5) and i <= (cline_x[j]+20):
							dwbc_tr[k,lbrei] = dwbc_tr[k,lbrei] + inn.vke[ik,j,i]*dlxv[j,i]*ddue[ik,j,i]

write.write_depth_meridional("transport_meridional_full2.nc",full_tr,'transport_mer_full',y,z)
write.write_depth_meridional("transport_meridional_dwbc2.nc",dwbc_tr,'transport_mer_dwbc',y,z)
